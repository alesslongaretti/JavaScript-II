...
let runnersLargeSizeShirt = runners.filter(function(size){
...
let ticketPriceTotal = runners.reduce(function(total, result) {
  return total += result.donation;
}, 0);
...
// Problem 1 people that work for the company Skinix
let runnerscompanyname = runners.filter(function(company){
  return company.company_name === 'Skinix';
});

console.log(runnerscompanyname);

// Problem 2 lowercase
let runnerskeys = runners.map (function (name) {
  return name.first_name.toLowerCase();
});

console.log(runnerskeys);

// Problem 3 donated more than 200

let runnershighdonation = runners.filter (function (money){
  return money.donation >= 200;
});

console.log(runnershighdonation);